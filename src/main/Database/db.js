...
export function seedDatabase() {
  ...
  if (!fs.existsSync(dbPath)){
        ...
        for (let res_id = 1; res_id <= 27; res_id++) {
            ...
            while (vacationWeeks.size < 4) vacationWeeks.add(Math.floor(Math.random() * 52))
        ...
        // --- Seed schedule sets & schedule ---
        const scheduleSetInfo = db.prepare(`
            INSERT INTO schedule_sets (name, start_date, end_date) VALUES (?, ?, ?)
